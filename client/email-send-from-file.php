...
if (isset($_POST['submit'])){
    ...
    $email_limit=ORM::for_table('accounts')->find_one($cid);
    $email_count=$email_limit['email_limit'];

    if($listscount>0){

    if($email_count>=$listscount){
        ...
        $sysEmail=$email_limit['email'];
        $sysCompany=$email_limit['name'].' '.$email_limit['lname'];
        ...
        if ($send=='1'){
            ...
            conf($self,'e','This Email Template is Not Active');
        }


        $remain_email=$email_count-$listscount;

        $email_limit->email_limit=$remain_email;
        $email_limit->save();

        conf('email-history.php','s','Email Send Successfully');
    }else{
        conf($self,'e','You do not have enough email balance to send these emails' );
    }
    }else{
        conf($self,'e','There Have no Email Address In this file' );
    }
}
