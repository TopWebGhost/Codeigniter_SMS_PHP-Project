...
if (isset($_POST['submit'])){
    ...
    $emailfound = ORM::for_table('accounts')->where('email', $email)->count();
    ...
    if ($emailfound=='1'){
        $fprand = _raid('16');
        $fptime = time()+3600;
        $ef = ORM::for_table('accounts')->where('email', $email)->find_one();
        $name = $ef['name'].' '.$ef['lname'];
        ...
        $d= ORM::for_table('email_templates')->where('tplname', 'Forgot Client Password')->find_one();
        ...
        $fpw_link = "$sysUrl/client/forgot_password.php?_token=$fprand";
...
if (isset($_GET['_token'])){
    ...
    $tfnd = ORM::for_table('accounts')->where('pwresetkey', $token)->count();


    if ($tfnd=='1'){

        $d = ORM::for_table('accounts')->where('pwresetkey', $token)->find_one();
        $cid = $d['id'];
        $name = $d['name'].' '.$d['lname'];
        $email = $d['email'];
        $rawpass = _raid(16);
        ...
        $d= ORM::for_table('email_templates')->where('tplname', 'Client Password Reset')->find_one();
        $template = $d['message'];
        $subject = $d['subject'];
        $send = $d['send'];
        $data = array('name' => $name,
