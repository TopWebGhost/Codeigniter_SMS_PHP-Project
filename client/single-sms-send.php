...
if (isset($_POST['submit'])){
    ...
    if($cmd!=''){
        $f_c_sms=ORM::for_table('accounts')->where('parent',$cid)->find_one($cmd);
        if($f_c_sms){
            $clphone=$f_c_sms['phone'];
        }else{
            conf($self,'e','Please Insert Phone Number');
        }
    }else{
        $clphone=_post('receiver');

        if($clphone==''){
            conf($self,'e','Please Insert Receiver Phone Number');
        }
    }
    ...
    if($sender_id!=''){
        $f_sender=ORM::for_table('manage_sender')->where('s_id',$sender_id)->where('status','0')->find_one();
        if($f_sender){
            conf($self,'e','This Sender ID have Blocked By Administrator');
        }
    }
    ...
    $sms_count=$clresult['sms_limit'];

    $phone=str_replace('+','',$clphone);

    $c_code=PhoneNumber::get_code($phone);

    $sms_cost=ORM::for_table('int_country_codes')->where('country_code',$c_code)->where('active','1')->find_one();

    if($sms_cost){
        $sms_charge=$sms_cost['tariff'];
    }else{
        conf($self,'e','Phone Number Coverage are not active');
    }

    if($sms_count>=$sms_charge){
        ...
        if($gateway=='Clickatell'){
            ...
            if($api_count=='1'){
                ...
                if ($sess[0] == "OK") {
                    ...
                    $message=urlencode($message);
                    $url = "$sms_url/http/sendmsg?session_id=$sess_id&to=$clphone&text=$message&mo=1&from=$sender_id";
        ...
        if($gateway=='Telenorcsms'){
         ...
         conf('send-sms.php','e','You Do Not Have Enough Balance to sent this SMS');
        }
    ...
    $remain_sms=$sms_count-$sms_charge;
    ...
    conf($self,'s','Please Check SMS History');
    }else{
